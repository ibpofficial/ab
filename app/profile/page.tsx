    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#090d16] text-[#f3f4f6]">
        ...
        <main className="flex-1 max-w-3xl w-full mx-auto px-4 sm:px-6 py-12 flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-xs text-slate-400 font-mono">Loading Profile Settings...</p>
        </main>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col bg-[#090d16] text-[#f3f4f6]">
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <Card className="p-6 bg-slate-900/90 border-slate-800 shadow-xl space-y-4 rounded-xl text-white">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 rounded-xl border-2 border-amber-500/60 bg-amber-500/10 flex items-center justify-center font-black text-amber-400 text-xl shadow-inner">
                {getInitials(name || "Student")}
              </div>

              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h1 className="text-2xl font-extrabold text-white">{name || "Student Builder"}</h1>
                ...
                <p className="text-xs text-slate-400 font-medium mt-0.5 flex items-center gap-2">
                  ...
                  <span className="font-mono text-slate-400">ID: {user?.uid.substring(0, 10)}...</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Link href={`/u/${user?.uid || "student-2"}`}>
                <Button variant="primary" size="sm" className="rounded-xl text-xs py-2 px-4 shadow-md">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card className="p-4 bg-slate-900/90 border-slate-800 shadow-md text-center rounded-xl text-white">
          ...
          <Card className="p-4 bg-slate-900/90 border-slate-800 shadow-md text-center rounded-xl text-white">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Longest Streak</div>
            <div className="text-xl font-black text-white mt-1">{student?.longestStreak || 0} Days</div>
          </Card>
          <Card className="p-4 bg-slate-900/90 border-slate-800 shadow-md text-center rounded-xl text-white">
            <div className="text-[10px] text-slate-400 uppercase font-bold">Completed</div>
            <div className="text-xl font-black text-emerald-400 mt-1">{student?.completedDays || 0}/60</div>
          </Card>
        </div>
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-amber-500/40"
                />
              </div>

              {/* Field 2: College Name */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-300">College / University Name *</label>
                <input
                  type="text"
                  required
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs font-bold focus:outline-none focus:ring-2 focus:ring-amber-500/40"
              >
                {MOCK_TRACKS.map((t) => (
            ...
            <div className="pt-2">
              <Button
                ...
                size="md"
                disabled={saving}
                className="py-3 px-6 rounded-xl font-bold shadow-md"
              >
                {saving ? (
                  <>
        ...
        <Card className="p-6 bg-slate-900/90 border-slate-800 shadow-xl space-y-4 rounded-xl text-white">
          <h3 className="text-base font-extrabold text-white flex items-center gap-2 border-b border-slate-800 pb-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            ...
            {isAnonymous ? (
              <Button
                ...
                className="text-xs py-2 px-4 border-slate-700 shrink-0"
              >
              ...
              <Button
                ...
                size="sm"
                onClick={signOut}
                className="text-xs py-2 px-4 border-rose-500/40 text-rose-400 hover:bg-rose-500/10 shrink-0"
              >
